      ...
      >
        ...
        <div className="relative z-10">
          ...
          <div className="flex items-start justify-between mb-6">
            <div className={`p-3 rounded-xl bg-gradient-to-r ${project.color} bg-opacity-10`} style={{ background: 'rgba(59, 130, 246, 0.1)' }}>
          ...
          {/* Title & Description */}
          <h3 className="text-white text-2xl font-bold mb-3 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-blue-400 group-hover:to-cyan-400 transition-all duration-300">
            {project.title}
          </h3>
          <p className="text-space-200 text-sm leading-relaxed mb-6">
            {project.description}
          </p>
          ...
          <ul className="space-y-2 mb-8">
            {project.highlights.map((h, i) => (
              <li key={i} className="flex gap-2 text-space-300 text-sm">
          ...
          <div className="flex flex-wrap gap-2">
            {project.tech.map((t) => (
              <span
                key={t}
                className="px-3 py-1 text-xs font-mono font-medium rounded-full bg-space-700 text-blue-300 border border-space-500"
              >
                {t}
              </span>
            ))}
...
export default function Projects() {
  ...
  return (
    <section id="projects" ref={sectionRef} className="relative min-h-screen flex items-center py-20 md:py-32 px-6 snap-start">
      {/* Ambient glow */}
      <div className="absolute top-1/3 left-0 w-[400px] h-[400px] rounded-full bg-blue-500/5 blur-[120px] pointer-events-none" />
      ...
      <div className="max-w-6xl mx-auto w-full">
        ...
        <div className="reveal opacity-0 transition-all duration-700 translate-y-8 [&.animate-in]:opacity-100 [&.animate-in]:translate-y-0 mb-16">
          ...
          <p className="text-space-300 text-lg max-w-2xl mt-4">
        ...
        {/* More from GitHub — horizontal scroll */}
        {repos.length > 0 && (
          <div className="reveal opacity-0 transition-all duration-700 delay-500 translate-y-8 [&.animate-in]:opacity-100 [&.animate-in]:translate-y-0 mt-16">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-white text-xl font-semibold">More from GitHub</h3>
            ...
            <div className="flex gap-4 overflow-x-auto overflow-y-visible pt-2 pb-4 snap-x snap-mandatory -mx-6 px-6 scrollbar-thin">
              {repos.map((repo) => (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="snap-start shrink-0 w-72 glass-card rounded-xl p-5 hover:border-blue-500/40 transition-all group"
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {repo.stargazers_count > 0 && (
                      <span className="text-space-400 text-xs flex items-center gap-1">
        ...
        {/* GitHub link */}
        <div className="reveal opacity-0 transition-all duration-700 delay-700 translate-y-8 [&.animate-in]:opacity-100 [&.animate-in]:translate-y-0 text-center mt-12">
          <a
